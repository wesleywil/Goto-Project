...
const AnimeSearch = ({hidden, setHidden})=>{
    ...
    const handleCreateAnimeDocument = (anime)=>{
        createAnimeDocument(anime).then(()=>{
                ...
                <AnimeSearchList>
                    {(listAnime.length)?
                        listAnime.slice(0,9).map((anime)=>(
                            ...
                            <AnimeCard key={anime.mal_id} name={anime.title} background={anime.images.webp.image_url} handle={()=>handleCreateAnimeDocument({anime})} cardHidden={'block'} buttonName={'Add'}/>
                        ))
                    :
